...
import type { NodeStatus, FirewallNodeData } from '@/types';
import { defaultFirewallData } from '@/types';

export type { FirewallNodeData };

interface FirewallNodeProps {
  data: FirewallNodeData;
  selected?: boolean;
}

const SIGNAL_SECURITY = 'oklch(0.65 0.20 25)';

function FirewallNode({ data, selected }: FirewallNodeProps) {
  const {
    label,
    status = 'idle',
    defaultAction = defaultFirewallData.defaultAction,
    allowedPorts = defaultFirewallData.allowedPorts,
    inspectionLatencyMs = defaultFirewallData.inspectionLatencyMs,
  } = data;
    ...
    >
      ...
      <div className="flex items-center justify-between px-3 py-2">
        <div className="flex items-center gap-2">
          <ShieldOff className="w-3.5 h-3.5" style={{ color: SIGNAL_SECURITY }} />
      ...
      <div className="px-3 pb-2.5 space-y-1">
        <div className="flex items-center justify-between font-mono text-xs">
          <span className="font-semibold" style={{ color: SIGNAL_SECURITY }}>
